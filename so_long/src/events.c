...
void    event_manager(t_game *a)
{
    ...
    while (++a->index_coll <= a->count_coll)
        if (a->player.x == a->coll[a->index_coll].x && a->player.y == a->coll[a->index_coll].y)
        {
            ...
            a->count_coll--;
            printf("coll restants : %zu\n", a->count_coll);
        }
    a->index_monster = -1;
    while (++a->index_monster < a->count_monster)
        if (a->player.x == a->monster[a->index_monster].x && a->player.y == a->monster[a->index_monster].y)
          close_game(a->mlx, 2);
}
void    monster_ia(t_game *a)
{
    ...
    while (++a->index_monster < a->count_monster)
    {
        ...
        if (move_done != 1) 
        {
            if (a->player.x < a->monster[a->index_monster].x && check_monster_move(a,line, -1) == 1)
                monster_move(a, 'L');
            if (a->player.x > a->monster[a->index_monster].x && check_monster_move(a,line, 1) == 1)
                monster_move(a, 'R');
            if ((check_monster_move(a,line, -1) != 1 && check_monster_move(a,line_up, 0) != 1) && move_done != 1)
